...
@Slf4j
@Component
public class PostgresBackupParser implements BackupParser {
    ...
    public BackupData parsearArchivo(MultipartFile file) throws Exception {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((linea = reader.readLine()) != null) {
                ...
                } else if (enCopy && !linea.trim().isEmpty()) {
                    contenidoActual.append(linea).append("\n");
                }
            }

        } catch (Exception e) {
            log.error("Error procesando backup PostgreSQL", e);
            throw new Exception("Error al procesar backup PostgreSQL: " + e.getMessage());
        }
    ...
    private String extraerNombreTabla(String linea) {
        Matcher matcher = COPY_PATTERN.matcher(linea);
        if (matcher.find()) {
