...
@Slf4j
@Component
public class SqlServerBackupParser implements BackupParser {
    private static final Pattern INSERT_PATTERN = Pattern.compile("INSERT \\[dbo\\]\\.\\[(.*?)\\]");
    ...
    public BackupData parsearArchivo(MultipartFile file) throws Exception {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            log.error("Error procesando backup SQL Server", e);
            throw new Exception("Error al procesar backup SQL Server: " + e.getMessage());
        }
    ...
    private String extraerNombreTabla(String linea) {
        Matcher matcher = INSERT_PATTERN.matcher(linea);
        if (matcher.find()) {
    ...
    private List<String[]> extraerValores(String contenido) {
        ...
        while (matcher.find()) {
            ...
            for (int i = 0; i < fila.length; i++) {
                fila[i] = fila[i].trim().replaceAll("^N?'|'$", "");
            }
            valores.add(fila);
        }
        return valores;
